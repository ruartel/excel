...
$areas_result = DB::query('SELECT * from kehalim');
$areas=array();
...
$cols_result = DB::query('SHOW COLUMNS FROM project2kahal');
$x='A';
$colsName = array();
...
$row=2;
$result = DB::query('SELECT * FROM project2kahal');
foreach ($result as $k => $inst){
    $x='A';
    foreach($inst as $k=>$v){
        if(($k == 'kahal_id1' || $k == 'kahal_id2') && $inst[$colsName[$x]] > 0){
