...
$cols_result = DB::query('SHOW COLUMNS FROM institutes');
$x='A';
$colsName = array();
foreach($cols_result as $col){
    if($x == 'A'){
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue($x . '1', 'inst_id');
    }else{
...
$row=2;
$result = DB::query('SELECT * FROM institutes');
foreach ($result as $k => $inst){
    $x='A';
    foreach($inst as $k=>$v){
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue($x . $row, $inst[$colsName[$x]]);
        $x++;
    }
    ++$row;
}
...
//header('Content-Disposition: attachment;filename="matan1.xls"');
