...
$areas_result = DB::query('SELECT * from areas');
$areas=array();
...
$cols_result = DB::query('SHOW COLUMNS FROM institute2area');
$x='A';
$colsName = array();
...
$row=2;
$result = DB::query('SELECT * FROM institute2area');
foreach ($result as $k => $inst){
    $x='A';
    foreach($inst as $k=>$v){
        if($k == 'area_id'){
            $value = $areas[$inst[$colsName[$x]]];
        }else{
...
//header('Content-Disposition: attachment;filename="matan2.xls"');
